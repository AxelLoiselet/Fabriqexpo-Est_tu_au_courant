  ...
  const int BParcade[7] ={5,4,3,2,6,7,8}; //pins associé aux boutons arcade dans l'ordre suivant: interupteur,ampoule,fil,pile,mode junior,mode pro ,mode genie
  ...
  #define PanneFil 10
  #define PanneInteru 11
  #define PanneAmpoule 12
  

  #define InterruON 13 //permet de savoir si l'interupteur est en ON ou OFF
...
//partie DFplayer
  ...
  myDFPlayer.volume(10);  //regle le volume de 0 à 30
  //En utilisant myDFPlayer.play(x) la valeur x correspond à un des fichier mp3 enregistré sur la carte microSD
  //les mp3 actuelement sur la carte correspondent à dans l'ordre  1:son d'activation ; 2: son de trompette defaite ; 3:son de felicitation ; 4:son d'erreur ; 5:son d'initialisation; 6: son de victoire
...
void loop() {
 ...
 delay(50);
  ...
  else
  {
    ...
    for (int i=0;i<4;i++) //chaque itération verifie un bouton à la suite
    {
      if(digitalRead(BParcade[i])==LOW)// si le bouton que l'on verifie est pressé
      {
        if(i==3)//si ce bouton est celui des pile
        {
         ...
         myDFPlayer.play(6);// Son de felicitation
    ...
    if(randNumber==0 || digitalRead(InterruON) ==LOW) // ou si l'interupteur est sur off
    {
     digitalWrite(PanneInteru,HIGH);
    }
    else
    {
      digitalWrite(PanneInteru, LOW);// Panne désactivé
    }
    if (randNumber==1)
    {
    ...
    if (randNumber==0 || randNumber2 == 0 || digitalRead(InterruON) ==LOW)//panne de l'interrupteur qui survient en fonction du nombre aléatoire généré
    {
     digitalWrite(PanneInteru,HIGH);
    }
    else
  ...
  case 4:// sous-etat du mode genie qui attend la deuxieme bonne reponse
    ...
    //allume la led de la panne précedement trouvé
...
//si on appuie sur un bouton d'erreur le timer est remis à 0
  ...
  //allume l'ampoule si on est dans l'etat de selection de difficulté ET que les piles sont en place ET que l'interrupteur est sur ON
  if(etat==0 && digitalRead(pileIN) ==HIGH && digitalRead(InterruON) ==HIGH )
  {
